...
Host_System::Host_System(
  ...
  for (uint16_t flow_id = 0; flow_id < parameters->IO_Flow_Definitions.size();
    ...
    switch (
        ((SSD_Components::Host_Interface_NVMe*)ssd_host_interface)->GetType()) {
      case HostInterface_Types::NVME:
        nvme_sq_size =
            ((SSD_Components::Host_Interface_NVMe*)ssd_host_interface)
    ...
    switch (parameters->IO_Flow_Definitions[flow_id]->Type) {
      case Flow_Type::SYNTHETIC: {
        ...
        io_flow = new Host_Components::IO_Flow_Synthetic(
            this->ID() + ".IO_Flow.Synth.No_" + std::to_string(flow_id),
            flow_id,
            Utils::Logical_Address_Partitioning_Unit::
                Start_lha_available_to_flow(flow_id),
            ...
            this->SATA_hba, parameters->Enable_ResponseTime_Logging,
      ...
      case Flow_Type::TRACE: {
        ...
        io_flow = new Host_Components::IO_Flow_Trace_Based(
            ...
            flow_param->Priority_Class,
            flow_param->Initial_Occupancy_Percentage / double(100.0),
            flow_param->File_Path, flow_param->Time_Unit,
            flow_param->Relay_Count, flow_param->Percentage_To_Be_Executed,
            ssd_host_interface->GetType(), this->PCIe_root_complex,
            this->SATA_hba, parameters->Enable_ResponseTime_Logging,
            parameters->ResponseTime_Logging_Period_Length,
            parameters->Input_file_path + ".IO_Flow.No_" +
  ...
  switch (ssd_device->Host_interface->GetType()) {
    ...
    case HostInterface_Types::SATA:
      ((SSD_Components::Host_Interface_SATA*)ssd_device->Host_interface)
          ->Set_ncq_address(SATA_hba->Get_sata_ncq_info()
                                ->Submission_queue_memory_base_address,
