...
Host_System::Host_System(Host_Parameter_Set *parameters, bool preconditioning_required, SSD_Components::Host_Interface_Base *ssd_host_interface) : MQSimEngine::Sim_Object("Host"), preconditioning_required(preconditioning_required)
{
...
void Host_System::Start_simulation()
{
  ...
  case HostInterface_Types::NVME:
    ...
    {
      ((SSD_Components::Host_Interface_NVMe *)ssd_device->Host_interface)->Create_new_stream(IO_flows[flow_cntr]->Priority_class(), IO_flows[flow_cntr]->Get_start_lsa_on_device(), IO_flows[flow_cntr]->Get_end_lsa_address_on_device(), IO_flows[flow_cntr]->Get_nvme_queue_pair_info()->Submission_queue_memory_base_address, IO_flows[flow_cntr]->Get_nvme_queue_pair_info()->Completion_queue_memory_base_address);
    }
    break;
  case HostInterface_Types::SATA:
    ((SSD_Components::Host_Interface_SATA *)ssd_device->Host_interface)->Set_ncq_address(SATA_hba->Get_sata_ncq_info()->Submission_queue_memory_base_address, SATA_hba->Get_sata_ncq_info()->Completion_queue_memory_base_address);
  default:
    break;
  }
