...
class IO_Flow_Parameter_Set : public Parameter_Set_Base {
 public:
  ...
  Flow_Type Type;
  IO_Flow_Priority_Class::Priority Priority_Class;  // The priority class is only considered when the SSD
  ...
  flash_die_ID_type* Die_IDs;                       // Resource partitioning: which die ids are
                                                    // allocted to this flow
  flash_plane_ID_type* Plane_IDs;                   // Resource partitioning: which plane ids are
                                                    // allocated to this flow
  unsigned int Initial_Occupancy_Percentage;        // Percentage of the logical space
                                                    // that is written when
                                                    // preconditioning is performed
...
class IO_Flow_Parameter_Set_Synthetic : public IO_Flow_Parameter_Set {
 public:
  ...
  unsigned int Total_Requests_To_Generate;  // If Stop_Time is equal to zero, then requst
                                            // generator considers
