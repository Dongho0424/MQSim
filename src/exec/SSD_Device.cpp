    ...
    case NVM::NVM_Type::FLASH: {
      ...
      switch (parameters->Flash_Parameters.Flash_Technology) {
        case Flash_Technology_Type::SLC:
          ...
          read_latencies[0] =
              parameters->Flash_Parameters.Page_Read_Latency_LSB;
        ...
        case Flash_Technology_Type::MLC:
          ...
          read_latencies[0] =
              parameters->Flash_Parameters.Page_Read_Latency_LSB;
          read_latencies[1] =
              parameters->Flash_Parameters.Page_Read_Latency_MSB;
          ...
          average_flash_read_latency =
              (read_latencies[0] + read_latencies[1]) / 2;
          average_flash_write_latency =
        ...
        case Flash_Technology_Type::TLC:
          ...
          read_latencies[0] =
              parameters->Flash_Parameters.Page_Read_Latency_LSB;
          read_latencies[1] =
              parameters->Flash_Parameters.Page_Read_Latency_CSB;
          read_latencies[2] =
              parameters->Flash_Parameters.Page_Read_Latency_MSB;
          ...
          average_flash_write_latency =
              (write_latencies[0] + write_latencies[1] + write_latencies[2]) /
              3;
          break;
        default:
      ...
      switch (parameters->Flash_Comm_Protocol) {
        case SSD_Components::ONFI_Protocol::NVDDR2: {
          SSD_Components::ONFI_Channel_NVDDR2** channels = new SSD_Components::
              ONFI_Channel_NVDDR2*[parameters->Flash_Channel_Count];
          for (unsigned int channel_cntr = 0;
               channel_cntr < parameters->Flash_Channel_Count; channel_cntr++) {
            NVM::FlashMemory::Flash_Chip** chips =
                new NVM::FlashMemory::Flash_Chip*[parameters
                                                      ->Chip_No_Per_Channel];
        ...
        switch (parameters->HostInterface_Type) {
          ...
          case HostInterface_Types::NVME: {
            ...
            for (int j = 0; j < (*io_flows)[i]->Chip_No; j++) {
              flow_chip_id_assignments[i].push_back(
                  (*io_flows)[i]->Chip_IDs[j]);
            }
            std::vector<flash_die_ID_type> die_ids;
            flow_die_id_assignments.push_back(die_ids);
            ...
            for (int j = 0; j < (*io_flows)[i]->Plane_No; j++) {
              flow_plane_id_assignments[i].push_back(
                  (*io_flows)[i]->Plane_IDs[j]);
            }
            break;
          }
          default:
...
LPA_type SSD_Device::Convert_host_logical_address_to_device_address(
    LHA_type lha) {
