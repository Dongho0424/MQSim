...
void read_configuration_parameters(const string ssd_config_file_path, Execution_Parameter_Set* exec_params) {
  ...
  if (!ssd_config_file) {
    ...
    string line((std::istreambuf_iterator<char>(ssd_config_file)), std::istreambuf_iterator<char>());
  ...
  if (!workload_defs_file) {
    ...
    if (line.compare("USE_INTERNAL_PARAMS") != 0) {
      ...
      if (mqsim_io_scenarios != NULL) {
        for (auto xml_io_scenario = mqsim_io_scenarios->first_node("IO_Scenario"); xml_io_scenario;
...
int main(int argc, char* argv[]) {
  ...
  for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {
    ...
    Host_System host(&exec_params->Host_Configuration, exec_params->SSD_Device_Configuration.Enabled_Preconditioning,
                     ssd.Host_interface);
