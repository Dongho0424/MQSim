...
inline void Input_Stream_Manager_NVMe::Handle_new_arrived_request(
...
inline void Input_Stream_Manager_NVMe::Handle_arrived_write_data(
    User_Request* request) {
  segment_user_request(request);
  ((Host_Interface_NVMe*)host_interface)
      ->broadcast_user_request_arrival_signal(request);
...
inline void Input_Stream_Manager_NVMe::Handle_serviced_request(
    User_Request* request) {
...
IO_Flow_Priority_Class::Priority Input_Stream_Manager_NVMe::Get_priority_class(
    stream_id_type stream_id) {
...
void Request_Fetch_Unit_NVMe::Process_pcie_read_message(
  ...
  switch (dma_req_item->Type) {
    case DMA_Req_Type::REQUEST_INFO: {
      ...
      new_request->Stream_id =
          (stream_id_type)((uint64_t)(dma_req_item->object));
      ...
      switch (sqe->Opcode) {
        case NVME_READ_OPCODE:
          ...
          new_request->Start_LBA =
              ...
              (LHA_type)sqe->Command_specific[0];  // Command Dword 10 and
                                                   // Command Dword 11
