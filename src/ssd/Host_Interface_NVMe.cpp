...
Input_Stream_NVMe::~Input_Stream_NVMe() {
  for (auto& user_request : Waiting_user_requests) delete user_request;
  for (auto& user_request : Completed_user_requests) delete user_request;
}
...
stream_id_type Input_Stream_Manager_NVMe::Create_new_stream(
    IO_Flow_Priority_Class::Priority priority_class, LHA_type start_logical_sector_address,
    LHA_type end_logical_sector_address, uint64_t submission_queue_base_address, uint16_t submission_queue_size,
  ...
  Input_Stream_NVMe* input_stream = new Input_Stream_NVMe(
      priority_class, start_logical_sector_address, end_logical_sector_address, submission_queue_base_address,
      submission_queue_size, completion_queue_base_address, completion_queue_size);
  this->input_streams.push_back(input_stream);

  return (stream_id_type)(this->input_streams.size() - 1);
}

inline void Input_Stream_Manager_NVMe::Submission_queue_tail_pointer_update(stream_id_type stream_id,
...
inline void Input_Stream_Manager_NVMe::Completion_queue_head_pointer_update(stream_id_type stream_id,
...
inline void Input_Stream_Manager_NVMe::Handle_serviced_request(User_Request* request) {
  ...
  inform_host_request_completed(stream_id, request);  // Completion queue is not full, so the device can
...
inline void Input_Stream_Manager_NVMe::inform_host_request_completed(stream_id_type stream_id, User_Request* request) {
  ((Request_Fetch_Unit_NVMe*)((Host_Interface_NVMe*)host_interface)->request_fetch_unit)
      ->Send_completion_queue_element(request,
                                      ((Input_Stream_NVMe*)input_streams[stream_id])->Submission_head_informed_to_host);
  ...
  while (handled_sectors_count < req_size) {
    ...
    if (user_request->Type == UserRequestType::READ) {
      NVM_Transaction_Flash_RD* transaction = new NVM_Transaction_Flash_RD(
          Transaction_Source_Type::USERIO, user_request->Stream_id, transaction_size * SECTOR_SIZE_IN_BYTE, lpa, NO_PPA,
          user_request, user_request->Priority_class, 0, access_status_bitmap, CurrentTimeStamp);
      ...
      NVM_Transaction_Flash_WR* transaction = new NVM_Transaction_Flash_WR(
          Transaction_Source_Type::USERIO, user_request->Stream_id, transaction_size * SECTOR_SIZE_IN_BYTE, lpa,
          user_request, user_request->Priority_class, 0, access_status_bitmap, CurrentTimeStamp);
      user_request->Transaction_list.push_back(transaction);
...
void Request_Fetch_Unit_NVMe::Process_pcie_read_message(uint64_t address, void* payload, unsigned int payload_size) {
  ...
  switch (dma_req_item->Type) {
    case DMA_Req_Type::REQUEST_INFO: {
      ...
      new_request->Priority_class = ((Input_Stream_Manager_NVMe*)host_interface->input_stream_manager)
...
Host_Interface_NVMe::Host_Interface_NVMe(const sim_object_id_type& id, LHA_type max_logical_sector_address,
...
stream_id_type Host_Interface_NVMe::Create_new_stream(IO_Flow_Priority_Class::Priority priority_class,
  ...
  return ((Input_Stream_Manager_NVMe*)input_stream_manager)
      ->Create_new_stream(priority_class, start_logical_sector_address, end_logical_sector_address,
                          submission_queue_base_address, submission_queue_depth, completion_queue_base_address,
...
uint16_t Host_Interface_NVMe::Get_submission_queue_depth() { return submission_queue_depth; }

uint16_t Host_Interface_NVMe::Get_completion_queue_depth() { return completion_queue_depth; }
