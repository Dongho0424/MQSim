...
class Cached_Mapping_Table {
 public:
  ...
  page_status_type Get_bitmap_vector_of_written_sectors(
      const stream_id_type streamID, const LPA_type lpa);
...
class Address_Mapping_Unit_Page_Level : public Address_Mapping_Unit_Base {
 ...
 public:
  ...
  NVM::FlashMemory::Physical_Page_Address Convert_ppa_to_address(
      const PPA_type ppa);
 ...
 private:
  ...
  void allocate_page_in_plane_for_user_write(
  ...
  void generate_flash_read_request_for_mapping_data(
  ...
  void manage_user_transaction_facing_barrier(
      NVM_Transaction_Flash* transaction);
