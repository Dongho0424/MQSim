...
enum class GC_Block_Selection_Policy_Type {
  ...
  RGA, /*The randomized-greedy algorithm described in: "B. Van Houdt, A Mean
     Field Model for a Class of Garbage Collection Algorithms in Flash - based
     Solid State Drives, SIGMETRICS, 2013" and "Stochastic Modeling of
     Large-Scale Solid-State Storage Systems: Analysis, Design Tradeoffs and
     Optimization, SIGMETRICS, 2013".*/
  RANDOM,
  RANDOM_P,
  RANDOM_PP, /*The RANDOM, RANDOM+, and RANDOM++ algorithms described in: "B.
Van Houdt, A Mean Field Model  for a Class of Garbage Collection Algorithms in
...
class GC_and_WL_Unit_Base : public MQSimEngine::Sim_Object {
 public:
  GC_and_WL_Unit_Base(const sim_object_id_type& id, Address_Mapping_Unit_Base* address_mapping_unit,
                      ...
                      bool preemptible_gc_enabled, double gc_hard_threshold, unsigned int channel_count,
  ...
  virtual void Check_gc_required(const unsigned int BlockPoolSize,
                                 const NVM::FlashMemory::Physical_Page_Address& planeAddress) = 0;
  GC_Block_Selection_Policy_Type Get_gc_policy();
  unsigned int Get_GC_policy_specific_parameter();  // Returns the parameter specific to the
 ...
 protected:
  ...
  bool is_safe_gc_wl_candidate(
      ...
      const flash_block_ID_type gc_wl_candidate_block_id);  // Checks if block_address is a safe
                                                            // candidate for gc execution, i.e., 1) it
                                                            // is not a write frontier, and 2) there
                                                            // is no ongoing program operation
  ...
  unsigned int max_ongoing_gc_reqs_per_plane;  // This value has two important usages: 1)
                                               // maximum number of concurrent gc
                                               // operations per plane, and 2) the value
                                               // that determines urgent GC execution
                                               // when there is a shortage of flash
                                               // blocks. If the block bool size drops
                                               // below this value, all incomming user
