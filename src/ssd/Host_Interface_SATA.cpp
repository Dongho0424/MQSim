...
Input_Stream_Manager_SATA::Input_Stream_Manager_SATA(Host_Interface_Base* host_interface, uint16_t ncq_depth,
  ...
  Input_Stream_SATA* input_stream =
      new Input_Stream_SATA(start_logical_sector_address, end_logical_sector_address, 0, 0);
  this->input_streams.push_back(input_stream);
}

void Input_Stream_Manager_SATA::Set_ncq_address(uint64_t submission_queue_base_address,
                                                uint64_t completion_queue_base_address) {
  ((Input_Stream_SATA*)this->input_streams[SATA_STREAM_ID])->Submission_queue_base_address =
      submission_queue_base_address;
  ((Input_Stream_SATA*)this->input_streams[SATA_STREAM_ID])->Completion_queue_base_address =
...
inline void Input_Stream_Manager_SATA::Handle_serviced_request(User_Request* request) {
  ...
  if (((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Submission_head !=
    ...
    ((Host_Interface_SATA*)host_interface)->request_fetch_unit->Fetch_next_request(SATA_STREAM_ID);
    ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->On_the_fly_requests++;
    ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Submission_head++;  // Update submission queue head after
  ...
  if (((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Completion_head >
    ...
    if (((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Completion_tail + 1 ==
  ...
  } else if (((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Completion_tail -
                 ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Completion_head ==
             ncq_depth - 1) {
    ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])
        ->Completed_user_requests.push_back(request);  // Wait while the completion queue is full
  ...
  inform_host_request_completed(request);  // Completion queue is not full, so the device can DMA the
...
inline void Input_Stream_Manager_SATA::inform_host_request_completed(User_Request* request) {
  ((Request_Fetch_Unit_SATA*)((Host_Interface_SATA*)host_interface)->request_fetch_unit)
      ->Send_completion_queue_element(
          request, ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Submission_head_informed_to_host);
...
void Input_Stream_Manager_SATA::segment_user_request(User_Request* user_request) {
  ...
  while (handled_sectors_count < req_size) {
    ...
    if (lsa < ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Start_logical_sector_address ||
        lsa > ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->End_logical_sector_address) {
      lsa = ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Start_logical_sector_address +
            (lsa % (((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->End_logical_sector_address -
                    (((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])->Start_logical_sector_address)));
    }
    LHA_type internal_lsa = lsa - ((Input_Stream_SATA*)input_streams[SATA_STREAM_ID])
    ...
    if (user_request->Type == UserRequestType::READ) {
      NVM_Transaction_Flash_RD* transaction = new NVM_Transaction_Flash_RD(
          Transaction_Source_Type::USERIO, SATA_STREAM_ID, transaction_size * SECTOR_SIZE_IN_BYTE, lpa, NO_PPA,
          user_request, 0, access_status_bitmap, CurrentTimeStamp);
      ...
      NVM_Transaction_Flash_WR* transaction = new NVM_Transaction_Flash_WR(
          Transaction_Source_Type::USERIO, SATA_STREAM_ID, transaction_size * SECTOR_SIZE_IN_BYTE, lpa, user_request, 0,
      ...
      switch (sqe->Opcode) {
        case SATA_READ_OPCODE:
          ...
          new_request->Start_LBA =
              ((LHA_type)sqe->Command_specific[1]) << 31 | (LHA_type)sqe->Command_specific[0];  // Command Dword 10 and
                                                                                                // Command Dword 11
...
void Request_Fetch_Unit_SATA::Fetch_next_request(stream_id_type stream_id) {
  ...
  host_interface->Send_read_message_to_host(
      im->Submission_queue_base_address + im->Submission_head * sizeof(Submission_Queue_Entry),
...
void Request_Fetch_Unit_SATA::Send_completion_queue_element(User_Request* request, uint16_t sq_head_value) {
  ...
  host_interface->Send_write_message_to_host(
      im->Completion_queue_base_address + im->Completion_tail * sizeof(Completion_Queue_Entry), cqe,
      sizeof(Completion_Queue_Entry));
  number_of_sent_cqe++;
  if (number_of_sent_cqe % ncq_depth == 0) {
    if (current_phase == 0xffff) {  // According to protocol specification, the
                                    // value of the Phase Tag is inverted each
                                    // pass through the Completion Queue
...
Host_Interface_SATA::Host_Interface_SATA(const sim_object_id_type& id, const uint16_t ncq_depth,
                                         const LHA_type max_logical_sector_address, const unsigned int sectors_per_page,
                                         Data_Cache_Manager_Base* cache)
    : Host_Interface_Base(id, HostInterface_Types::SATA, max_logical_sector_address, sectors_per_page, cache),
      ncq_depth(ncq_depth) {
...
void Host_Interface_SATA::Set_ncq_address(const uint64_t submission_queue_base_address,
                                          const uint64_t completion_queue_base_address) {
  ((Input_Stream_Manager_SATA*)this->input_stream_manager)
      ->Set_ncq_address(submission_queue_base_address, completion_queue_base_address);
}
