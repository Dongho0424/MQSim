...
namespace SSD_Components
{
  ...
  inline void Input_Stream_Manager_SATA::Handle_serviced_request(User_Request *request)
  {
    ...
    else if (((Input_Stream_SATA *)input_streams[SATA_STREAM_ID])->Completion_tail - ((Input_Stream_SATA *)input_streams[SATA_STREAM_ID])->Completion_head == ncq_depth - 1)
    {
      ((Input_Stream_SATA *)input_streams[SATA_STREAM_ID])->Completed_user_requests.push_back(request); // Wait while the completion queue is full
      return;
    }
  ...
  void Input_Stream_Manager_SATA::segment_user_request(User_Request *user_request)
  {
    ...
    while (handled_sectors_count < req_size)
    {
      ...
      {
        lsa = ((Input_Stream_SATA *)input_streams[SATA_STREAM_ID])->Start_logical_sector_address + (lsa % (((Input_Stream_SATA *)input_streams[SATA_STREAM_ID])->End_logical_sector_address - (((Input_Stream_SATA *)input_streams[SATA_STREAM_ID])->Start_logical_sector_address)));
      }
  ...
  Request_Fetch_Unit_SATA::Request_Fetch_Unit_SATA(Host_Interface_Base *host_interface, uint16_t ncq_depth) : Request_Fetch_Unit_Base(host_interface), current_phase(0xffff), number_of_sent_cqe(0), ncq_depth(ncq_depth) {}
  ...
  Host_Interface_SATA::Host_Interface_SATA(const sim_object_id_type &id,
                                           const uint16_t ncq_depth, const LHA_type max_logical_sector_address, const unsigned int sectors_per_page, Data_Cache_Manager_Base *cache) : Host_Interface_Base(id, HostInterface_Types::SATA, max_logical_sector_address, sectors_per_page, cache), ncq_depth(ncq_depth)
  {
