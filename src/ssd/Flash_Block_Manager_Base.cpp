...
unsigned int Block_Pool_Slot_Type::bitmap_size = 0;
Flash_Block_Manager_Base::Flash_Block_Manager_Base(GC_and_WL_Unit_Base* gc_and_wl_unit,
                                                   ...
                                                   unsigned int stream_count, unsigned int channel_count,
  ...
  for (unsigned int C = 0; C < channel_count; C++) {
    plane_manager[C] = new PlaneBookKeepingType**[chip_no_per_channel];
    for (unsigned int W = 0; W < chip_no_per_channel; W++) {
      plane_manager[C][W] = new PlaneBookKeepingType*[die_no_per_chip];
      for (unsigned int D = 0; D < die_no_per_chip; D++) {
        plane_manager[C][W][D] = new PlaneBookKeepingType[plane_no_per_die];

        // Initialize plane book keeping data structure
        for (unsigned int P = 0; P < plane_no_per_die; P++) {
          PlaneBookKeepingType* plane = &plane_manager[C][W][D][P];
          plane->Total_pages_count = block_no_per_plane * pages_no_per_block;
          plane->Free_pages_count = block_no_per_plane * pages_no_per_block;
          plane->Valid_pages_count = 0;
          plane->Invalid_pages_count = 0;
          plane->Ongoing_erase_operations.clear();
          plane->Blocks = new Block_Pool_Slot_Type[block_no_per_plane];

          // Initialize block pool for plane
          for (unsigned int B = 0; B < block_no_per_plane; B++) {
            Block_Pool_Slot_Type* block = &plane->Blocks[B];
            block->BlockID = B;
            ...
            // ceiling devision; 64 = uint64_t의 bit 개수
            Block_Pool_Slot_Type::bitmap_size = (pages_no_per_block + 63) / 64;
            block->Invalid_page_bitmap = new uint64_t[Block_Pool_Slot_Type::bitmap_size];
            for (unsigned int i = 0; i < Block_Pool_Slot_Type::bitmap_size; i++) {
              block->Invalid_page_bitmap[i] = All_VALID_PAGE;
            }
            plane->Add_to_free_block_pool(&plane->Blocks[B], false);
          }

          plane->Data_wf = new Block_Pool_Slot_Type*[stream_count];
          plane->Translation_wf = new Block_Pool_Slot_Type*[stream_count];
          plane->GC_wf = new Block_Pool_Slot_Type*[stream_count];
          for (int i = 0; i < stream_count; i++) {
            plane->Data_wf[i] = plane->Get_a_free_block(i, false);
            plane->Translation_wf[i] = plane->Get_a_free_block(i, true);
            plane->GC_wf[i] = plane->Get_a_free_block(i, false);
          }
        }
      }
    }
    ...
    for (unsigned int W = 0; W < chip_no_per_channel; W++) {
      for (unsigned int D = 0; D < die_no_per_chip; D++) {
        for (unsigned int P = 0; P < plane_no_per_die; P++) {
          PlaneBookKeepingType* plane = &plane_manager[C][W][D][P];
          for (unsigned int B = 0; B < block_no_per_plane; B++) {
            delete[] plane->Blocks[B].Invalid_page_bitmap;
          }
          delete[] plane->Blocks;
          delete[] plane->GC_wf;
          delete[] plane->Data_wf;
          delete[] plane->Translation_wf;
        }
        delete[] plane_manager[C][W][D];
      }
      delete[] plane_manager[C][W];
    }
    delete[] plane_manager[C];
  }
...
void PlaneBookKeepingType::Check_bookkeeping_correctness(const NVM::FlashMemory::Physical_Page_Address& plane_address) {
  ...
  if (Free_pages_count == 0) {
    PRINT_ERROR("Plane " << "@" << plane_address.ChannelID << "@" << plane_address.ChipID << "@" << plane_address.DieID
                         << "@" << plane_address.PlaneID << " pool size: " << Get_free_block_pool_size()
                         << " ran out of free pages! Bad resource management! "
                            "It is not safe to continue simulation!");
  }
}

unsigned int PlaneBookKeepingType::Get_free_block_pool_size() { return (unsigned int)Free_block_pool.size(); }
...
bool Flash_Block_Manager_Base::Can_execute_gc_wl(const NVM::FlashMemory::Physical_Page_Address& block_address) {
  PlaneBookKeepingType* plane_record =
      &plane_manager[block_address.ChannelID][block_address.ChipID][block_address.DieID][block_address.PlaneID];
  return (plane_record->Blocks[block_address.BlockID].Ongoing_user_program_count +
...
void Flash_Block_Manager_Base::program_transaction_issued(const NVM::FlashMemory::Physical_Page_Address& page_address) {
  PlaneBookKeepingType* plane_record =
      &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
