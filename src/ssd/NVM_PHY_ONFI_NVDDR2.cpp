...
inline BusChannelStatus NVM_PHY_ONFI_NVDDR2::Get_channel_status(
    flash_channel_ID_type channelID) {
  return channels[channelID]->GetStatus();
}

inline NVM::FlashMemory::Flash_Chip* NVM_PHY_ONFI_NVDDR2::Get_chip(
    flash_channel_ID_type channelID, flash_chip_ID_type chipID) {
...
void NVM_PHY_ONFI_NVDDR2::Send_command_to_chip(
  ...
  switch (transaction_list.front()->Type) {
    case Transaction_Type::READ:
      ...
      break;
    case Transaction_Type::WRITE:
      if (((NVM_Transaction_Flash_WR*)transaction_list.front())
        ...
        if (transaction_list.size() == 1) {
          ...
          DEBUG("Chip "
                << targetChip->ChannelID << ", " << targetChip->ChipID << ", "
                << transaction_list.front()->Address.DieID
        ...
        if (_my_instance->channels[chip->ChannelID]->GetStatus() ==
          ...
          switch (dieBKE->ActiveTransactions.front()->Source) {
            ...
            case Transaction_Source_Type::MAPPING:
              _my_instance->WaitingMappingRead_TX[chip->ChannelID].push_back(
                  (*it));
              break;
          }
        }
    ...
    case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
      ...
      if (_my_instance->channels[chip->ChannelID]->GetStatus() ==
        ...
        Simulator->Register_sim_event(
            ...
            (int)NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED);
        chipBKE->OngoingDieCMDTransfers.push(dieBKE);
        _my_instance->channels[chip->ChannelID]->SetStatus(
            BusChannelStatus::BUSY, chip);
...
inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(
    ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE,
