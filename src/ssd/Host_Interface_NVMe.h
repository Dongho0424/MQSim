...
class Input_Stream_NVMe : public Input_Stream_Base {
 public:
  Input_Stream_NVMe(IO_Flow_Priority_Class::Priority priority_class, LHA_type start_logical_sector_address,
                    LHA_type end_logical_sector_address, uint64_t submission_queue_base_address,
                    uint16_t submission_queue_size, uint64_t completion_queue_base_address,
  ...
  uint16_t Submission_head_informed_to_host;  // To avoide race condition, the
                                              // submission head must not be informed
                                              // to host until the request info of
                                              // the head is successfully arrived
  uint16_t Submission_tail;
  uint16_t Completion_head;
  uint16_t Completion_tail;
  std::list<User_Request*> Waiting_user_requests;         // The list of requests that have been fetch to
                                                          // the device queue and are getting serviced
  std::list<User_Request*> Completed_user_requests;       // The list of requests that are completed but
                                                          // have not been informed to the host due to
                                                          // full CQ
  std::list<User_Request*> Waiting_write_data_transfers;  // The list of write requests that are
                                                          // waiting for data
  uint16_t On_the_fly_requests;                           // the number of requests that are either being
                                                          // fetch from host or waiting in the device queue
};
