...
class Block_Pool_Slot_Type {
 public:
  ...
  bool Hot_block = false;  // Used for hot/cold separation mentioned in the "On the necessity
...
class Flash_Block_Manager_Base {
 ...
 public:
  Flash_Block_Manager_Base(GC_and_WL_Unit_Base* gc_and_wl_unit, unsigned int max_allowed_block_erase_count,
  ...
  virtual void Allocate_block_and_page_in_plane_for_user_write(const stream_id_type streamID,
                                                               NVM::FlashMemory::Physical_Page_Address& address) = 0;
  virtual void Allocate_block_and_page_in_plane_for_gc_write(const stream_id_type streamID,
                                                             NVM::FlashMemory::Physical_Page_Address& address) = 0;
  virtual void Allocate_block_and_page_in_plane_for_translation_write(const stream_id_type streamID,
                                                                      NVM::FlashMemory::Physical_Page_Address& address,
                                                                      bool is_for_gc) = 0;
  virtual void Allocate_Pages_in_block_and_invalidate_remaining_for_preconditioning(
  ...
  virtual void Invalidate_page_in_block_for_preconditioning(const stream_id_type streamID,
                                                            const NVM::FlashMemory::Physical_Page_Address& address) = 0;
  ...
  // Checks if there is an ongoing gc for block_address
  bool Block_has_ongoing_gc_wl(const NVM::FlashMemory::Physical_Page_Address& block_address);

  // Checks if the gc request can be executed on
  // block_address (there shouldn't be any ongoing user
  // read/program requests targeting block_address)
  bool Can_execute_gc_wl(const NVM::FlashMemory::Physical_Page_Address& block_address);

  // Updates the block bookkeeping record
  ...
  // Cheks if block has any ongoing program request
  bool Is_having_ongoing_program(const NVM::FlashMemory::Physical_Page_Address& block_address);
  // Make the page invalid in the block bookkeeping record
  bool Is_page_valid(Block_Pool_Slot_Type* block, flash_page_ID_type page_id);
 ...
 protected:
  ...
  void program_transaction_issued(
      const NVM::FlashMemory::Physical_Page_Address& page_address);  // Updates the block bookkeeping record
};
}  // namespace SSD_Components
