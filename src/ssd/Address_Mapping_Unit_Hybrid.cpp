...
Address_Mapping_Unit_Hybrid::Address_Mapping_Unit_Hybrid(
    sim_object_id_type id, FTL* ftl, NVM_PHY_ONFI* flash_controller, Flash_Block_Manager_Base* block_manager,
    bool ideal_mapping_table, unsigned int concurrent_streams_no, unsigned int channel_count,
    unsigned int chip_no_per_channel, unsigned int die_no_per_chip, unsigned int plane_no_in_die,
    unsigned int block_no_per_plane, unsigned int page_no_per_block, unsigned int sectors_per_page,
    unsigned int page_size_in_byte, double overprovisioning_ratio, CMT_Sharing_Mode sharing_mode,
    bool fold_out_of_range_addresses)
    : Address_Mapping_Unit_Base(id, ftl, flash_controller, block_manager, ideal_mapping_table, concurrent_streams_no,
                                channel_count, chip_no_per_channel, die_no_per_chip, plane_no_in_die,
                                block_no_per_plane, page_no_per_block, sectors_per_page, page_size_in_byte,
...
void Address_Mapping_Unit_Hybrid::Allocate_address_for_preconditioning(const stream_id_type stream_id,
...
void Address_Mapping_Unit_Hybrid::Get_data_mapping_info_for_gc(const stream_id_type stream_id, const LPA_type lpa,
                                                               PPA_type& ppa, page_status_type& page_state) {}

void Address_Mapping_Unit_Hybrid::Get_translation_mapping_info_for_gc(const stream_id_type stream_id,
...
LPA_type Address_Mapping_Unit_Hybrid::Get_logical_pages_count(stream_id_type stream_id) { return 0; }

void Address_Mapping_Unit_Hybrid::Convert_ppa_to_address(const PPA_type ppa,
...
void Address_Mapping_Unit_Hybrid::Allocate_new_page_for_gc(NVM_Transaction_Flash_WR* transaction,
...
void Address_Mapping_Unit_Hybrid::manage_mapping_transaction_facing_barrier(stream_id_type stream_id, MVPN_type mvpn,
                                                                            bool read) {}
