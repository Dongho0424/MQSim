...
void FTL::Perform_precondition(
  ...
  for (auto& stat : workload_stats) {
    ...
    if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
      ...
      switch (decision_dist_type) {
        case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
          random_hot_address_generator = new Utils::RandomGenerator(
              stat->random_hot_address_generator_seed);
      ...
      stat->Address_distribution_type = Utils::Address_Distribution_Type::
          RANDOM_HOTCOLD;  // Initially assume that the trace has hot/cold
                           // access pattern
    ...
    switch (decision_dist_type) {
      ...
      {
        double r_to_f_ratio = std::sqrt(
            double(stat->Ratio_of_traffic_accessing_hot_region) /
        ...
        switch (GC_and_WL_Unit->Get_gc_policy()) {
          case GC_Block_Selection_Policy_Type::
              GREEDY:  // Based on: B. Van Houdt, "A mean field model for a
                       // class of garbage collection algorithms in flash-based
                       // solid state drives", SIGMETRICS 2013.
          case GC_Block_Selection_Policy_Type::
              FIFO:  // Could be estimated with greedy for large
          ...
          case GC_Block_Selection_Policy_Type::
              RGA:  // Based on: B. Van Houdt, "A mean field model for a class
          ...
          case GC_Block_Selection_Policy_Type::
              RANDOM_P:  // Based on: B. Van Houdt, "A mean field model for a
          ...
          case GC_Block_Selection_Policy_Type::
              RANDOM_PP:  // Based on: B. Van Houdt, "A mean field model for a
                          // class of garbage collection algorithms in
                          // flash-based solid state drives", SIGMETRICS 2013.
          {
            ...
            for (int i = page_no_per_block; i >= 0; i--) {
              if (i <=
                  int(GC_and_WL_Unit->Get_GC_policy_specific_parameter())) {
                steadystate_block_status_probability[i] =
                     ...
                     (rho /
                      (1 - rho -
                       mu_b * (page_no_per_block * S_rho_b - page_no_per_block +
                               GC_and_WL_Unit
                                   ->Get_GC_policy_specific_parameter()))));
        ...
        case CMT_Sharing_Mode::SHARED: {
          ...
          if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
            switch (stat->generator_type) {
              case Utils::Request_Generator_Type::BANDWIDTH:
                flow_rate = 1.0 /
                            double(stat->Average_inter_arrival_time_nano_sec) *
                            SIM_TIME_TO_SECONDS_COEFF *
                            stat->Average_request_size_sector;
                break;
              case Utils::Request_Generator_Type::QUEUE_DEPTH: {
                sim_time_type max_arrival_time = sim_time_type(
                    stat->Read_ratio * double(avg_flash_read_latency) +
                    (1 - stat->Read_ratio) * double(avg_flash_program_latency));
                double avg_arrival_time = double(max_arrival_time) /
                                          double(stat->Request_queue_depth);
        ...
        case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
          ...
          unsigned int required_no_of_hot_cmt_entries =
              (unsigned int)(stat->Ratio_of_hot_addresses_to_whole_working_set *
                             no_of_entries_in_cmt);
          unsigned int entries_to_bring_into_cmt =
              required_no_of_hot_cmt_entries;
