...
FTL::FTL(const sim_object_id_type& id, Data_Cache_Manager_Base* data_cache_manager, unsigned int channel_no,
         ...
         sim_time_type avg_flash_read_latency, sim_time_type avg_flash_program_latency, double over_provisioning_ratio,
  ...
  Stats::Init_stats(channel_no, chip_no_per_channel, die_no_per_chip, plane_no_per_die, block_no_per_plane,
                    page_no_per_block, max_allowed_block_erase_count);
}

FTL::~FTL() {
  Stats::Clear_stats(channel_no, chip_no_per_channel, die_no_per_chip, plane_no_per_die, block_no_per_plane,
                     page_no_per_block, max_allowed_block_erase_count);
}

void FTL::Validate_simulation_config() {
  NVM_Firmware::Validate_simulation_config();
  if (this->Data_cache_manager == NULL) throw std::logic_error("The cache manager is not set for FTL!");
  if (this->Address_Mapping_Unit == NULL) throw std::logic_error("The mapping module is not set for FTL!");
  if (this->BlockManager == NULL) throw std::logic_error("The block manager is not set for FTL!");
  if (this->GC_and_WL_Unit == NULL) throw std::logic_error("The garbage collector is not set for FTL!");
}
void FTL::Perform_precondition(std::vector<Utils::Workload_Statistics*> workload_stats) {
    ...
    if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
      switch (stat->generator_type) {
        case Utils::Request_Generator_Type::BANDWIDTH:
          overall_rate += 1.0 / double(stat->Average_inter_arrival_time_nano_sec) * SIM_TIME_TO_SECONDS_COEFF *
                          stat->Average_request_size_sector;
          break;
        case Utils::Request_Generator_Type::QUEUE_DEPTH: {
          sim_time_type max_arrival_time = sim_time_type(stat->Read_ratio * double(avg_flash_read_latency) +
        ...
        default:
          PRINT_ERROR(
              "Unknown request type generator in the FTL preconditioning "
              "function.")
      }
    } else {
      overall_rate += 1.0 / double(stat->Average_inter_arrival_time_nano_sec) * SIM_TIME_TO_SECONDS_COEFF *
  ...
  for (auto& stat : workload_stats) {
    LPA_type no_of_logical_pages_in_steadystate =
        (LPA_type)(stat->Initial_occupancy_ratio * Address_Mapping_Unit->Get_logical_pages_count(stat->Stream_id));
    ...
    if (stat->generate_aligned_addresses) {
      if (min_lha % stat->alignment_value != 0) min_lha += stat->alignment_value - (min_lha % stat->alignment_value);
      if (max_lha % stat->alignment_value != 0) max_lha -= min_lha % stat->alignment_value;
    }

    LPA_type max_lpa = Convert_host_logical_address_to_device_address(max_lha) - 1;
    total_accessed_cmt_entries +=
        (unsigned int)(Convert_host_logical_address_to_device_address(max_lha) / page_size_in_sectors -
    ...
    LHA_type hot_region_end_lsa = 0,
             hot_lha_used_for_generation = 0;  // Used for fast address generation in hot/cold traffic mode
    ...
    if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
      ...
      switch (decision_dist_type) {
        case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
          ...
          hot_region_end_lsa =
              min_lha + (LHA_type)((double)(max_lha - min_lha) * stat->Ratio_of_hot_addresses_to_whole_working_set);
          last_hot_lpa = Convert_host_logical_address_to_device_address(hot_region_end_lsa) -
                         1;  // Be conservative and not include the last_hot_address itself
          hot_lha_used_for_generation = min_lha;
          // Check if enough LPAs could be generated within the working set of
          // the flow
          if ((last_hot_lpa - min_lpa) < no_of_logical_pages_in_steadystate) fully_include_hot_addresses = true;
          if ((max_lpa - last_hot_lpa) < 1.1 * (no_of_logical_pages_in_steadystate - (last_hot_lpa - min_lpa))) {
            PRINT_MESSAGE(
                "The specified initial occupancy value could not be satisfied "
                "as the working set of workload #"
                << stat->Stream_id << " is small. MQSim made some adjustments!");
      ...
      while (lpa_set_for_preconditioning.size() < no_of_logical_pages_in_steadystate) {
        if (random_request_type_generator->Uniform(0, 1) <= stat->Read_ratio) is_read = true;
        ...
        switch (stat->Request_size_distribution_type) {
          ...
          case Utils::Request_Size_Distribution_Type::NORMAL: {
            double temp_request_size =
                random_request_size_generator->Normal(stat->Average_request_size_sector, stat->STDEV_reuqest_size);
        ...
        switch (decision_dist_type) {
          case Utils::Address_Distribution_Type::STREAMING:
            ...
            if (start_LBA + size > max_lha) start_LBA = min_lha;
            streaming_next_address += size;
            if (streaming_next_address > max_lha) streaming_next_address = min_lha;
            if (stat->generate_aligned_addresses)
            ...
            break;
          case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
            if (fully_include_hot_addresses)  // just to speedup address
                                              // generation
            {
              if (!hot_range_finished) {
                start_LBA = hot_lha_used_for_generation;
                hot_lha_used_for_generation += size;
                if (start_LBA > hot_region_end_lsa) hot_range_finished = true;
                is_hot_address = true;
              } else {
                ...
                if (start_LBA < hot_region_end_lsa + 1 || start_LBA > max_lha)
                  PRINT_ERROR(
                      "Out of range address is generated in "
                      "IO_Flow_Synthetic!\n")
                if (start_LBA + size > max_lha) start_LBA = hot_region_end_lsa + 1;
        ...
        if (stat->generate_aligned_addresses) start_LBA -= start_LBA % stat->alignment_value;
        ...
        LPA_type max_lpa_within_device = Convert_host_logical_address_to_device_address(stat->Max_LHA) -
    ...
    // Trace-based Workload
      ...
      for (auto itr = stat->Read_address_access_pattern.begin(); itr != stat->Read_address_access_pattern.end();
           itr++) {
      ...
      for (auto itr = stat->Write_address_access_pattern.begin(); itr != stat->Write_address_access_pattern.end();
           itr++) {
        LPA_type lpa = (*itr).first;
        if (lpa_set_for_preconditioning.size() < no_of_logical_pages_in_steadystate) {
          if (lpa_set_for_preconditioning.find(lpa) == lpa_set_for_preconditioning.end()) {
    ...
    switch (decision_dist_type) {
      ...
      case Utils::Address_Distribution_Type::RANDOM_HOTCOLD:  
      {
        ...
        switch (GC_and_WL_Unit->Get_gc_policy()) {
          case GC_Block_Selection_Policy_Type::GREEDY:  // Based on: B. Van Houdt, "A mean field model for a
                                                        // class of garbage collection algorithms in flash-based
                                                        // solid state drives", SIGMETRICS 2013.
          case GC_Block_Selection_Policy_Type::FIFO:    // Could be estimated with greedy for large
                                                        // page_no_per_block values, as mentioned in //Based on: B.
                                                        // Van Houdt, "A mean field model for a class of garbage
                                                        // collection algorithms in flash-based solid state
                                                        // drives", SIGMETRICS 2013.
          {
            for (unsigned int i = 0; i <= page_no_per_block; i++) {
              steadystate_block_status_probability.push_back(Utils::Combination_count(page_no_per_block, i) *
                                                             std::pow(rho, i) *
                                                             std::pow(1 - rho, page_no_per_block - i));
            }
            Utils::Euler_estimation(steadystate_block_status_probability, page_no_per_block, rho, 30, 0.001,
                                    0.0000000001,
                                    10000);  // As specified in the SIGMETRICS 2013 paper, a larger
                                             // value for d-choices (the name of RGA in Van Houdt's
                                             // paper) will lead to results close to greedy. We use
                                             // d=30 to estimate steady-state of the greedy policy
                                             // with that of d-chioces.
            break;
          }
          case GC_Block_Selection_Policy_Type::RGA:  // Based on: B. Van Houdt, "A mean field model for a class
                                                     ...
                                                     // state drives", SIGMETRICS 2013.
          {
            ...
            Utils::Euler_estimation(steadystate_block_status_probability, page_no_per_block, rho,
                                    GC_and_WL_Unit->Get_GC_policy_specific_parameter(), 0.001, 0.0000000001, 10000);
            break;
          }
          case GC_Block_Selection_Policy_Type::RANDOM:
          case GC_Block_Selection_Policy_Type::RANDOM_P:  // Based on: B. Van Houdt, "A mean field model for a
                                                          // class of garbage collection algorithms in
                                                          // flash-based solid state drives", SIGMETRICS 2013.
          {
            ...
            for (int i = page_no_per_block; i > 0; i--) {
              steadystate_block_status_probability[i] =
                  steadystate_block_status_probability[i] - steadystate_block_status_probability[i - 1];
            }
            break;
          }
          case GC_Block_Selection_Policy_Type::RANDOM_PP:  // Based on: B. Van Houdt, "A mean field model for a
                                                           // class of garbage collection algorithms in
                                                           // flash-based solid state drives", SIGMETRICS 2013.
          {
            ...
            double a_r =
                page_no_per_block - GC_and_WL_Unit->Get_GC_policy_specific_parameter() - page_no_per_block * S_rho_b;
            ...
            for (int i = page_no_per_block; i >= 0; i--) {
              if (i <= int(GC_and_WL_Unit->Get_GC_policy_specific_parameter())) {
                steadystate_block_status_probability[i] =
                    ...
                    (i + (rho / (1 - rho -
                                 mu_b * (page_no_per_block * S_rho_b - page_no_per_block +
        ...
        for (int i = 0; i < int(page_no_per_block) + 1; i++) {
          ...
          if (phi_index < 0 || phi_index >= int(page_no_per_block)) {
            ...
            steadystate_block_status_probability[i] =
                r_to_f_ratio * steadystate_block_status_probability_temp[phi_index];
          }
        }
        double sum = 0;
        for (unsigned int i = 0; i <= page_no_per_block; i++) {
      ...
      case Utils::Address_Distribution_Type::STREAMING:  // A simple estimation based on: "Stochastic Modeling of
                                                         // Large-Scale Solid-State Storage Systems: Analysis,
                                                         // Design Tradeoffs and Optimization", SIGMETRICS 2013
        for (unsigned int i = 0; i <= page_no_per_block; i++) {
          if (i == 0) {
      ...
      default:
        PRINT_ERROR(
            "Unhandled address distribution type in FTL's preconditioning "
            "function.")
    }
    ...
    if (sum > 1.001 || sum < 0.99) {
      PRINT_ERROR(
          "Wrong probability distribution function for the number of valid "
          "pages in flash blocks in the steady-state! It is not safe to "
          "continue preconditioning!")
    }
    Address_Mapping_Unit->Allocate_address_for_preconditioning(stat->Stream_id, lpa_set_for_preconditioning,
      ...
      switch (Address_Mapping_Unit->Get_CMT_sharing_mode()) {
        case CMT_Sharing_Mode::SHARED: {
          ...
          if (stat->Type == Utils::Workload_Type::SYNTHETIC) {
            switch (stat->generator_type) {
              case Utils::Request_Generator_Type::BANDWIDTH:
                flow_rate = 1.0 / double(stat->Average_inter_arrival_time_nano_sec) * SIM_TIME_TO_SECONDS_COEFF *
                            stat->Average_request_size_sector;
                break;
              case Utils::Request_Generator_Type::QUEUE_DEPTH: {
                sim_time_type max_arrival_time =
                    sim_time_type(stat->Read_ratio * double(avg_flash_read_latency) +
                                  (1 - stat->Read_ratio) * double(avg_flash_program_latency));
                double avg_arrival_time = double(max_arrival_time) / double(stat->Request_queue_depth);
          ...
          no_of_entries_in_cmt =
              (unsigned int)(double(flow_rate) / double(overall_rate) * Address_Mapping_Unit->Get_cmt_capacity());
          break;
        }
        case CMT_Sharing_Mode::EQUAL_SIZE_PARTITIONING:
          no_of_entries_in_cmt =
              (unsigned int)(1.0 / double(workload_stats.size()) * Address_Mapping_Unit->Get_cmt_capacity());
        ...
        default:
          PRINT_ERROR(
              "Unknown mapping table sharing mode in the FTL preconditioning "
              "function.")
      }
      ...
      switch (decision_dist_type) {
        case Utils::Address_Distribution_Type::RANDOM_HOTCOLD: {
          ...
          unsigned int required_no_of_hot_cmt_entries =
              (unsigned int)(stat->Ratio_of_hot_addresses_to_whole_working_set * no_of_entries_in_cmt);
          ...
          while (Address_Mapping_Unit->Get_current_cmt_occupancy_for_stream(stat->Stream_id) <
                 entries_to_bring_into_cmt) {
            Address_Mapping_Unit->Bring_to_CMT_for_preconditioning(stat->Stream_id, (*itr).second);
            trace_lpas_sorted_histogram.erase(itr++);
          }

          // If there is free space in the CMT, then bring the remaining
          // addresses to CMT
        ...
        case Utils::Address_Distribution_Type::RANDOM_UNIFORM: {
          ...
          int random_step =
              random_generator.Uniform_uint(0, (uint32_t)(trace_lpas_sorted_histogram.size()) / no_of_entries_in_cmt);
          auto itr = trace_lpas_sorted_histogram.begin();
          while (Address_Mapping_Unit->Get_current_cmt_occupancy_for_stream(stat->Stream_id) < no_of_entries_in_cmt) {
            ...
            if (trace_lpas_sorted_histogram.size() > 1) {
              trace_lpas_sorted_histogram.erase(itr++);
              if (random_walker + random_step >= int(trace_lpas_sorted_histogram.size() - 1) ||
...
LPA_type FTL::Convert_host_logical_address_to_device_address(LHA_type lha) { return lha / page_size_in_sectors; }
