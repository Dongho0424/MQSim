...
class NVM_PHY_ONFI_NVDDR2 : public NVM_PHY_ONFI {
 public:
  ...
  NVM_Transaction_Flash* Is_chip_busy_with_stream(
      NVM_Transaction_Flash* transaction);
  bool Is_chip_busy(NVM_Transaction_Flash* transaction);
  void Change_memory_status_preconditioning(
 ...
 private:
  ...
  void perform_interleaved_cmd_data_transfer(
  ...
  static void handle_ready_signal_from_chip(
      NVM::FlashMemory::Flash_Chip* chip,
