...
#define SATA_UPDATE_SQ_TAIL(Q) \
  ...
  if (Q.Submission_queue_tail == Q.Submission_queue_size) Q.Submission_queue_tail = 0;
...
class SATA_HBA : MQSimEngine::Sim_Object {
 public:
  SATA_HBA(sim_object_id_type id, uint16_t ncq_size, sim_time_type hba_processing_delay,
 ...
 private:
  ...
  std::list<Host_IO_Request*> waiting_requests_for_submission;  // The I/O requests that are still
