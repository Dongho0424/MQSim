...
IO_Flow_Synthetic::IO_Flow_Synthetic(
    const sim_object_id_type& name, uint16_t flow_id, LHA_type start_lsa_on_device, LHA_type end_lsa_on_device,
    double working_set_ratio, uint16_t io_queue_id, uint16_t nvme_submission_queue_size,
    uint16_t nvme_completion_queue_size, IO_Flow_Priority_Class::Priority priority_class, double read_ratio,
    Utils::Address_Distribution_Type address_distribution, double hot_region_ratio,
    Utils::Request_Size_Distribution_Type request_size_distribution, unsigned int average_request_size,
    unsigned int variance_request_size, Utils::Request_Generator_Type generator_type,
    sim_time_type Average_inter_arrival_time_nano_sec, unsigned int average_number_of_enqueued_requests,
    bool generate_aligned_addresses, unsigned int alignment_value, int seed, sim_time_type stop_time,
    double initial_occupancy_ratio, unsigned int total_req_count, HostInterface_Types SSD_device_type,
    PCIe_Root_Complex* pcie_root_complex, SATA_HBA* sata_hba, bool enabled_logging, sim_time_type logging_period,
    ...
    : IO_Flow_Base(name, flow_id, start_lsa_on_device,
                   ...
                   io_queue_id, nvme_submission_queue_size, nvme_completion_queue_size, priority_class, stop_time,
                   initial_occupancy_ratio, total_req_count, SSD_device_type, pcie_root_complex, sata_hba,
                   enabled_logging, logging_period, logging_file_path),
  ...
  if (this->start_lsa_on_device > this->end_lsa_on_device) {
    throw std::logic_error(
        "Problem in IO Flow Synthetic, the start LBA address is greater than "
        "the end LBA address");
  }
  ...
  if (address_distribution == Utils::Address_Distribution_Type::RANDOM_HOTCOLD) {
    ...
    hot_region_end_lsa = this->start_lsa_on_device +
                         (LHA_type)((double)(this->end_lsa_on_device - this->start_lsa_on_device) * hot_region_ratio);
  }
...
Host_IO_Request* IO_Flow_Synthetic::Generate_next_request() {
  ...
  switch (address_distribution) {
    case Utils::Address_Distribution_Type::STREAMING:
      ...
      if (streaming_next_address == request->Start_LBA) {
        PRINT_MESSAGE(
            "Synthetic Message Generator: The same address is always repeated "
            "due to configuration parameters!")
      }
      break;
  ...
  DEBUG("* Host: Request generated - " << (request->Type == Host_IO_Request_Type::READ ? "Read, " : "Write, ") << "LBA:"
                                       << request->Start_LBA << ", Size_in_bytes:" << request->LBA_count << "")
...
void IO_Flow_Synthetic::Start_simulation() {
  ...
  if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
    Simulator->Register_sim_event(
        (sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this,
        0, 0);
  } else {
...
void IO_Flow_Synthetic::Execute_simulator_event(MQSimEngine::Sim_Event* event) {
  if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
    ...
    if (req != NULL) {
      ...
      Simulator->Register_sim_event(Simulator->Time() + (sim_time_type)random_time_interval_generator->Exponential(
