...
PCIe_Link::PCIe_Link(const sim_object_id_type& id, PCIe_Root_Complex* root_complex, PCIe_Switch* pcie_switch,
                     double lane_bandwidth_GBPs, int lane_count, int tlp_header_size, int tlp_max_payload_size,
...
void PCIe_Link::Set_root_complex(PCIe_Root_Complex* root_complex) { this->root_complex = root_complex; }

void PCIe_Link::Set_pcie_switch(PCIe_Switch* pcie_switch) { this->pcie_switch = pcie_switch; }
...
void PCIe_Link::Deliver(PCIe_Message* message) {
  switch (message->Destination) {
    case PCIe_Destination_Type::HOST:  // Message from SSD device to the host
      ...
      Simulator->Register_sim_event(Simulator->Time() + estimate_transfer_time(message), this,
  ...
  switch (destination) {
    case PCIe_Destination_Type::HOST:
      ...
      if (Message_buffer_toward_root_complex.size() > 0) {  // There are active transfers
        Simulator->Register_sim_event(
            Simulator->Time() + estimate_transfer_time(Message_buffer_toward_root_complex.front()), this,
            (void*)(intptr_t)PCIe_Destination_Type::HOST, static_cast<int>(PCIe_Link_Event_Type::DELIVER));
      }
      break;
    case PCIe_Destination_Type::DEVICE:
      ...
      if (Message_buffer_toward_ssd_device.size() > 0) {
        Simulator->Register_sim_event(
            Simulator->Time() + estimate_transfer_time(Message_buffer_toward_ssd_device.front()), this,
            (void*)(intptr_t)PCIe_Destination_Type::DEVICE, static_cast<int>(PCIe_Link_Event_Type::DELIVER));
      }
      break;
  }
}
