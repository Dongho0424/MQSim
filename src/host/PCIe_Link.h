...
namespace Host_Components
{
  ...
  enum class PCIe_Link_Event_Type
  {
    DELIVER
  };
  ...
  private:
    ...
    sim_time_type estimate_transfer_time(PCIe_Message *message)
    {
      ...
      {
        int total_transfered_bytes = (message->Payload_size / tlp_max_payload_size) * (tlp_max_payload_size + packet_overhead) + (message->Payload_size % tlp_max_payload_size == 0 ? 0 : message->Payload_size % tlp_max_payload_size + packet_overhead);
