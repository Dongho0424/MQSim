...
IO_Flow_Trace_Based::IO_Flow_Trace_Based(const sim_object_id_type& name, uint16_t flow_id, LHA_type start_lsa_on_device,
                                         ...
                                         unsigned int percentage_to_be_simulated, HostInterface_Types SSD_device_type,
    ...
    : IO_Flow_Base(name, flow_id, start_lsa_on_device, end_lsa_on_device, io_queue_id, nvme_submission_queue_size,
                   nvme_completion_queue_size, priority_class, 0, initial_occupancy_ratio, 0, SSD_device_type,
                   pcie_root_complex, sata_hba, enabled_logging, logging_period, logging_file_path),
...
Host_IO_Request* IO_Flow_Trace_Based::Generate_next_request() {
  ...
  // LBA 보정. start_lsa_on_device ~ end_lsa_on_device 사이에 들어오도록
...
void IO_Flow_Trace_Based::Execute_simulator_event(MQSimEngine::Sim_Event*) {
  ...
  if (STAT_generated_request_count < total_requests_to_be_generated) {
    ...
    Simulator->Register_sim_event(
        time_offset + std::strtoll(current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10), this);
  }
}
...
void IO_Flow_Trace_Based::Get_statistics(Utils::Workload_Statistics& stats,
  ...
  stats.Stream_id = io_queue_id - 1;  // In MQSim, there is a simple relation between stream
      ...
      if (line_splitted[ASCIITraceTypeColumn].compare(ASCIITraceWriteCode) == 0) {
        if (stats.Write_address_access_pattern.find(device_address) == stats.Write_address_access_pattern.end()) {
          ...
          stats.Write_address_access_pattern[device_address].Access_count =
              stats.Write_address_access_pattern[device_address].Access_count + 1;
          stats.Write_address_access_pattern[device_address].Accessed_sub_units =
              stats.Write_address_access_pattern[device_address].Accessed_sub_units | access_status_bitmap;
        }
        ...
        if (stats.Read_address_access_pattern.find(device_address) == stats.Read_address_access_pattern.end()) {
          ...
          stats.Read_address_access_pattern[device_address].Access_count =
              stats.Read_address_access_pattern[device_address].Access_count + 1;
          stats.Read_address_access_pattern[device_address].Accessed_sub_units =
              stats.Read_address_access_pattern[device_address].Accessed_sub_units | access_status_bitmap;
        }
