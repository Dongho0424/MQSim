...
#define NVME_SQ_FULL(Q)                                                                                           \
  (Q.Submission_queue_tail < Q.Submission_queue_size - 1 ? Q.Submission_queue_tail + 1 == Q.Submission_queue_head \
                                                         : Q.Submission_queue_head == 0)
#define NVME_UPDATE_SQ_TAIL(Q) \
  Q.Submission_queue_tail++;   \
  if (Q.Submission_queue_tail == Q.Submission_queue_size) nvme_queue_pair.Submission_queue_tail = 0;

class PCIe_Root_Complex;
class IO_Flow_Base : public MQSimEngine::Sim_Object, public MQSimEngine::Sim_Reporter {
 public:
  IO_Flow_Base(const sim_object_id_type& name, uint16_t flow_id, LHA_type start_lsa_on_device,
               LHA_type end_lsa_address_on_device, uint16_t io_queue_id, uint16_t nvme_submission_queue_size,
