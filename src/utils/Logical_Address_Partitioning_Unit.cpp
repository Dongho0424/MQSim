...
void Logical_Address_Partitioning_Unit::Allocate_logical_address_for_flows(
      ...
      for (flash_chip_ID_type chip_id = 0; chip_id < stream_chip_ids[stream_id].size(); chip_id++) {
        for (flash_die_ID_type die_id = 0; die_id < stream_die_ids[stream_id].size(); die_id++) {
          for (flash_plane_ID_type plane_id = 0; plane_id < stream_plane_ids[stream_id].size(); plane_id++) {
            ...
            resource_list[stream_channel_ids[stream_id][channel_id]][stream_chip_ids[stream_id][chip_id]]
                         [stream_die_ids[stream_id][die_id]][stream_plane_ids[stream_id][plane_id]]++;
          }
        }
      }
    }
  ...
  for (unsigned int stream_id = 0; stream_id < concurrent_stream_no; stream_id++) {
    ...
    for (flash_channel_ID_type channel_id = 0; channel_id < stream_channel_ids[stream_id].size(); channel_id++) {
      Logical_Address_Partitioning_Unit::stream_channel_ids[stream_id].push_back(
          stream_channel_ids[stream_id][channel_id]);
    }
  ...
  if (initialized) {
    switch (hostinterface_type) {
      case HostInterface_Types::SATA:  // It is not possible to differentiate
                                       // between streams inside the device when
                                       // a SATA host interface is used, so all
                                       // lha space is available to all streams
        return total_lha_no;
        break;
      case HostInterface_Types::NVME:
        return (end_lhas_per_flow[stream_id] - start_lhas_per_flow[stream_id] + 1);
