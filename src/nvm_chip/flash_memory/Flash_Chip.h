...
class Flash_Chip : public NVM_Chip {
 ...
 public:
  Flash_Chip(const sim_object_id_type&, flash_channel_ID_type channelID, flash_chip_ID_type localChipID,
             ...
             sim_time_type suspendEraseLatency, sim_time_type commProtocolDelayRead = 20,
             sim_time_type commProtocolDelayWrite = 0, sim_time_type commProtocolDelayErase = 0);
  ~Flash_Chip();
  ...
  void EndCMDDataInXfer(Flash_Command* command)  // End transferring write data of a group of
  ...
  LPA_type Get_metadata(flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id,
                        flash_page_ID_type page_id);  // A simplification to decrease the complexity of GC
                                                      // execution! The GC unit may need to know the metadata of
                                                      // a page to decide if a page is valid or invalid.
 private:
